...
async function lstat(){if(!q.classId||!q.courseId){ElMessage.warning('请选择班级和课程');st.value=false;return}try{const r=await getScoreStats({classId:q.classId,courseId:q.courseId,examType:q.examType||null});const s=r?.[0]||{};Object.assign(sd,{avg:s.avg_score,max:s.max_score,min:s.min_score,pr:s.pass_rate});await nextTick();if(!ch)ch=echarts.init(cr.value);const sc=rows.value.filter(r=>r.score!=null).map(r=>Number(r.score));const b={'0-59':0,'60-69':0,'70-79':0,'80-89':0,'90-100':0};sc.forEach(s=>{if(s<60)b['0-59']++;else if(s<70)b['60-69']++;else if(s<80)b['70-79']++;else if(s<90)b['80-89']++;else b['90-100']++});ch.setOption({tooltip:{trigger:'axis'},xAxis:{type:'category',data:Object.keys(b)},yAxis:{type:'value',name:'人数'},series:[{type:'bar',data:Object.values(b),itemStyle:{borderRadius:[6,6,0,0],color:new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#3366cc'},{offset:1,color:'#88aadd'}])},barWidth:'40%'}]},true)}catch(e){console.error('Chart error:',e)}}
